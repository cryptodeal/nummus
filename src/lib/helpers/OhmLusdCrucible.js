import { abi as OhmLusdCrucible } from '$lib/abi/OhmLusdCrucible.json';
import { abi as UniswapIERC20 } from '$lib/abi/UniswapIERC20.json';
import { getTokenPrice } from '$lib/helpers/index';
import { addresses } from '$lib/const/index';
import { Contract } from '@ethersproject/contracts';
import { ohm_lusd, lusd } from '$lib/helpers/AllBonds';

export const calcAludelDetes = async (networkID, provider) => {
	const crucibleAddress = addresses[networkID].CRUCIBLE_OHM_LUSD;
	const aludelContract = new Contract(crucibleAddress, OhmLusdCrucible, provider);
	...
	aludelData.rewardSchedules.map((rs) => {
		let rewardStart = parseFloat(rs.start.toString());
		let rewardDuration = parseFloat(rs.duration.toString());
	...
	let avgRemainingDuration =
		remainingDurations.reduce((a, b) => a + b, 0) / remainingDurations.length;
	...
	const rewardTokenContract = new Contract(aludelData.rewardToken, UniswapIERC20, provider);
	...
	const bonusTokensLength = await aludelContract.getBonusTokenSetLength();
	const bonusTokensLengthNumber = bonusTokensLength.toNumber();

	let bonusTokenUsdValues = [];
	...
	await Promise.all(
		Array.from(Array(bonusTokensLengthNumber)).map(async (_, idx) => {
			...
			const bonusTokenContract = new Contract(bonusTokenAddress, UniswapIERC20, provider);
			...
			const usdValueOfBonusToken =
				(balanceOfBonusToken / 10 ** bonusTokenDecimals) * valueOfBonusToken;
	...
	let lusdContract = new Contract(lusdContractAddress, UniswapIERC20, provider);

	let stakedOhm =
		(await rewardTokenContract.balanceOf(aludelData.stakingToken)) / 10 ** rewardTokenDecimals;
	...
	let stakingTokenContract = new Contract(aludelData.stakingToken, UniswapIERC20, provider);
	...
	let aludelTotalStakedTokensUsd =
		(aludelTotalStakedTokens / sushiTokenSupply) * totalStakedTokensUsd;
	...
	//console.log(averageApy)
	//console.log(aludelTotalStakedTokensUsd)
